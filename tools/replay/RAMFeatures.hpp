...
**
** The idea is to get the RAM state and each bit be a feature in the feature std::vector.
**
...
class RAMFeatures {
	...
	public:
 		...
 		*          - To avoid return huge std::vectors, this method is void and the appropriate
 		* std::vector is returned trough a parameter passed by reference.
 		* 
 		...
 		* @param std::vector<int>& features an empy std::vector that will be filled with the requested information,
		...
		/**
 		* It 'returns' a binary std::vector containing 1's where the feature is active. Ideally this
 		...
 		* @param ALEScreen &screen is the current game screen that one may use to extract features.
 		* @param ALERAM &ram is the current game RAM that one may use to extract features.
 		* @param std::vector<bool>& features an empy std::vector that will be filled with the requested information,
 		*        therefore it must be passed by reference. Its i-th position is TRUE if the i-th feature is active.
