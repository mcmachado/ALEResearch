...
BPROFeatures::BPROFeatures(Parameters *param){
    this->param = param;
    numColumns  = param->numColumns;
	numRows     = param->numRows;
	numColors   = param->numColors;

	if(this->param->subtractBackground){
        this->background = new Background(param);
    }

	//To get the total number of features:
    numBasicFeatures = this->param->numColumns * this->param->numRows * this->param->numColors;
	numRelativeFeatures = (2 * this->param->numColumns - 1) * (2 * this->param->numRows - 1) 
							* (1+this->param->numColors) * this->param->numColors/2;
    changed.clear();
		...
		for (int bx = 0; bx < numColumns; bx++) {
			...
			for (int x = xo; x < xo + blockWidth; x++){
				for (int y = yo; y < yo + blockHeight; y++){
					...
					if(!this->param->subtractBackground || (this->background->getPixel(y, x) != pixel)){
        ...
        for (int k = 0; k < whichColors[c1].size(); k++){
            for (int h = 0; h < whichColors[c1].size(); h++){
                ...
                if (newBproFeature && bproExistence[rowDelta][columnDelta]){
                    ...
                    features.push_back(numBasicFeatures + (numColors + numColors - c1 + 1) * c1 / 
                        2 * numRowOffsets * numColumnOffsets+rowDelta * numColumnOffsets + columnDelta);
                ...
                for (vector<tuple<int, int> >::iterator it1 = whichColors[c1].begin(); it1 != whichColors[c1].end(); it1++){
                    for (vector<tuple<int, int> >::iterator it2 = whichColors[c2].begin(); it2 != whichColors[c2].end(); it2++){
                        ...
                        if (bproExistence[rowDelta][columnDelta]){
                            ...
                            bproExistence[rowDelta][columnDelta] = false;
                            features.push_back(numBasicFeatures + (numColors + numColors - c1 + 1) * c1 /
                                2 * numRowOffsets * numColumnOffsets + 
                                (c2 - c1) * numRowOffsets * numColumnOffsets + rowDelta * numColumnOffsets + columnDelta);
                        }
                    }
                }
            }
...
void BPROFeatures::getActiveFeaturesIndices(const ALEScreen &screen, vector<int>& features){
