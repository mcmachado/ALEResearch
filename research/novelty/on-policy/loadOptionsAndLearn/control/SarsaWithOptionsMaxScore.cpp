...
#include "SarsaWithOptionsMaxScore.hpp"
#include "../../../../../src/common/Timer.hpp"

using namespace std;

SarsaWithOptionsMaxScore::SarsaWithOptionsMaxScore(ALEInterface& ale, Features *features, Parameters *param) : RLLearner(ale, features, param) {
	delta = 0.0;
...
SarsaWithOptionsMaxScore::~SarsaWithOptionsMaxScore(){}

void SarsaWithOptionsMaxScore::updateQValues(vector<int> &Features, vector<float> &QValues){
...
void SarsaWithOptionsMaxScore::updateReplTrace(int action, vector<int> &Features){
	//e <- gamma * lambda * e
...
void SarsaWithOptionsMaxScore::updateAcumTrace(int action, vector<int> &Features){
	//e <- gamma * lambda * e
	for(unsigned int a = 0; a < nonZeroElig.size(); a++){
		int numNonZero = 0;
	 	for(unsigned int i = 0; i < nonZeroElig[a].size(); i++){
...
void SarsaWithOptionsMaxScore::sanityCheck(){
	for(int i = 0; i < numTotalActions; i++){
...
void SarsaWithOptionsMaxScore::saveWeightsToFile(string suffix){
...
void SarsaWithOptionsMaxScore::loadWeights(){
	string line;
...
void SarsaWithOptionsMaxScore::learnPolicy(ALEInterface& ale, Features *features, vector<vector<vector<float> > > &learnedOptions){
