...
def receiveSample(features_current_state, action, reward,features_next_state,proba_action_bpolicy):
    global weights,e,aux_weights,beta,gamma,epsilon,numberOfFeatures,numberOfActions,u,v
    ...
    ones = np.ones(numberOfActions * numberOfFeatures);
    u = (ones - eta * phi_t * phi_t)*u + rho*phi_t*phi_t + (rho - 1)*gamma*lambd*(ones - eta*phi_t*phi_t)*v

    v = gamma * lambd * rho * (ones - eta*phi_t*phi_t)*v + rho*phi_t*phi_t

    alpha = np.zeros(numberOfActions * numberOfFeatures);
