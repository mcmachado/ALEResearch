...
do
    ...
    then
        ...
        echo "plot '< awk -vn=1000 -f average.awk ${file}.tmp' using 1:2 title 'mode ${mode} difficulty ${difficulty}' with lines ls 1" >> ${name}.printscript
    else
        ...
        next_num=$(($last_num+1))
        sed "s/lines ls ${last_num}$/lines ls ${last_num}, '< awk -vn=1000 -f average.awk ${escaped}' using 1:2 title 'mode ${mode} difficulty ${difficulty}' with lines ls ${next_num}/g" ttmp.printscript > ${name}.printscript
        rm ttmp.printscript
    fi
